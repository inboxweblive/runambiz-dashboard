...
/* Average, computed once rather than on every render of the
   heading. */
...
useEffect(() => {
  ...
  async function resolveStore() {
    ...
    /* 1. Explicit ?store= wins. Used on localhost and by the
          dashboard's preview links. */

    const fromQuery = params.get("store");

    if (fromQuery) {
      if (!cancelled) {
        setStoreSlug(fromQuery);
    ...
    /* 2. Path form: app.runambiz.com/store/<slug> */
    ...
    if (pathMatch) {
      if (!cancelled) {
        setStoreSlug(decodeURIComponent(pathMatch[1]));
        setResolvingHost(false);
      }
      return;
    }
    ...
    /* 3. Runambiz subdomain, if you ever enable wildcards.
          Kept so existing links don't break. */

    if (hostname.endsWith(".runambiz.com")) {

      const subdomain = hostname.replace(".runambiz.com", "");
    ...
    /* 4. A merchant's own domain. Anything that isn't ours
          and isn't localhost gets looked up by hostname.

          The anon RLS policy only exposes published stores
          with an active domain, so an unverified or
          unpublished store returns nothing and falls through
          to the normal "not available" error. */

    const isOurs =
      hostname.endsWith("runambiz.com") ||
      hostname === "localhost" ||
      hostname === "127.0.0.1";
    ...
    if (!isOurs) {
      ...
      const { data } = await supabase
        .from("businesses")
        .select("slug")
        .or(`custom_domain.eq.${bare},custom_domain.eq.www.${bare}`)
...
/* Lock the page behind the drawer, and close it on Escape.
   Put this with the other effects. */
...
const onRunambizSubdomain =
  hostname.endsWith(".runambiz.com") &&
  !["www.runambiz.com", "app.runambiz.com"].includes(hostname);


const activeSlug = business?.slug || storeSlug;


const storeHomeHref =
  onOwnDomain || onRunambizSubdomain
    ? "/"
    : hostname === "localhost" || hostname === "127.0.0.1"
      ? `/store.html?store=${activeSlug}`
      : `/store/${activeSlug}`;
...
>
  ...
  <button
    type="button"
    className="store-mobile-drawer-backdrop"
    aria-label="Close menu"
    onClick={() => setMenuOpen(false)}
  />
 
  <nav
    className="store-mobile-drawer-panel"
 ...
 <div className="store-products-heading">
          ...
          {(() => {
            ...
            const inner = (
              <>
                <img
                  src={business.featured_post_image_url}
                  alt={business.featured_post_title || ""}
                  loading="lazy"
                  decoding="async"
                />
                ...
                {(business.featured_post_eyebrow ||
                  ...
                  <div className="store-featured-post-copy">
 
                    {business.featured_post_eyebrow && (
                      <span>{business.featured_post_eyebrow}</span>
                    )}
 
                    {business.featured_post_title && (
                      <strong>{business.featured_post_title}</strong>
                    )}
            ...
            /* A link only when the merchant set one — an <a>
               with no href is not focusable and confuses
               screen readers. */
 
            return business.featured_post_link ? (
              <a
                href={business.featured_post_link}
                target="_blank"
                rel="noreferrer"
                className="store-featured-post-frame is-link"
              >
                {inner}
                <ArrowUpRight size={18} />
              </a>
            ) : (
              <div className="store-featured-post-frame">
                {inner}
              </div>
            );
  ...
  <section className="store-info-panel">
    ...
    {business.reviews_enabled !== false && (
 
          <section id="reviews" className="store-reviews-section">
              ...
              <div>
                ...
                {reviewSummary.count > 0 && (
                  <div className="store-reviews-summary">
                    <StoreStars value={reviewSummary.average} />
              ...
              <div className="store-reviews-grid">
                ...
                {reviews.map(review => (
                  ...
                  <article key={review.id} className="store-review-card">
                    ...
                    <StoreStars value={review.rating} />
    ...
    <div className="store-footer-brand">
      ...
      >
        ...
        {(business.contact_whatsapp ||
          ...
          >
            <MessageCircle size={16} />
            Message {business.name.split(/\s+/)[0]}
          </button>
...
function StoreStars({ value, size = 15 }) {
...
/* =========================================================
   ...
   Submits straight to the table. RLS allows the insert; a
   database trigger forces it into the pending state, so
   nothing reaches the storefront until the merchant
   approves it.
========================================================= */
 
function StoreReviewForm({ businessId, hasReviews }) {
  ...
  async function submit(event) {
    ...
    try {
      ...
      setError(
        err?.message || "We couldn't send your review. Please try again."
      );
  ...
  if (done) {
    ...
    return (
      <div className="store-review-thanks">
        <CheckCircle2 size={20} />
        <div>
          <strong>Thank you</strong>
          <span>
            Your review has been sent to the seller. It appears
            here once they approve it.
          </span>
        </div>
      </div>
    );
  ...
  return (
    ...
    <form className="store-review-form" onSubmit={submit}>
      ...
      <small>
        Your review is sent to the seller and appears here
...
function getDiscoveryStoreUrl(slug) {
  ...
  if (hostname === "localhost" || hostname === "127.0.0.1") {
    return `/store.html?store=${encodeURIComponent(slug)}`;
  }

  return `https://app.runambiz.com/store/${encodeURIComponent(slug)}`;
